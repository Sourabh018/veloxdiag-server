...
public class RecommendationService {
    ...
    private static final String MIGRATION_SYSTEM_PROMPT =
            "You are a senior backend engineer. You are given a slow SQL query and its EXPLAIN plan " +
            "(showing a sequential scan). Produce a ready-to-paste database migration script and a short " +
            "git commit message for the fix. Base the column choice only on the WHERE/JOIN/ORDER BY columns " +
            "actually visible in the SQL text — never invent a table or column name that isn't present in " +
            "the input. Use CREATE INDEX CONCURRENTLY where it makes sense for a production migration, and " +
            "include a one-line SQL comment above the statement explaining what it targets. Respond in this " +
            "exact format, no other text:\n" +
            "MIGRATION:\n<full migration file content>\n" +
            "COMMIT:\n<one-line conventional commit message>";

    public MigrationScriptResult generateMigrationScript(String endpoint, String ruleType) {
        if (!"MISSING_INDEX_CANDIDATE".equals(ruleType)) {
            return new MigrationScriptResult(endpoint, ruleType, null, null, false,
                    "Migration scripts are only generated for missing-index findings.");
        }
        ...
        if (seqScanPlan == null) {
            return new MigrationScriptResult(endpoint, ruleType, null, null, false,
                    "No captured query plan with a sequential scan for this endpoint yet.");
        }
        if (!keyRotator.hasKeys()) {
            return new MigrationScriptResult(endpoint, ruleType, null, null, false,
                    "Migration generation isn't configured (missing GROQ_API_KEY).");
        }

        try {
            String userPrompt = "SQL:\n" + seqScanPlan.getSqlText() +
                    "\n\nEXPLAIN PLAN:\n" + seqScanPlan.getExplainPlan();
            ObjectNode root = buildChatRequestBody(MIGRATION_SYSTEM_PROMPT, userPrompt, 600, 0.3);
            String requestBody = objectMapper.writeValueAsString(root);
            ...
            for (int i = 0; i < attempts; i++) {
                ...
                if (response.statusCode() == 200) {
                    ...
                    if (choices.isArray() && choices.size() > 0) {
                        String text = choices.get(0).path("message").path("content").asText();
                        String[] parsed = parseMigrationCommit(text);
                        if (parsed[0] != null) {
                            return new MigrationScriptResult(endpoint, ruleType, parsed[0], parsed[1], true, null);
                        }
                    }
                    break;
                }
        ...
        return new MigrationScriptResult(endpoint, ruleType, null, null, false,
                "Migration script generation failed — try again.");
    }

    // returns [migration, commit]
    private String[] parseMigrationCommit(String raw) {
        String migration = null;
        String commit = null;
        int migIdx = raw.indexOf("MIGRATION:");
        int commitIdx = raw.indexOf("COMMIT:");
        if (migIdx != -1 && commitIdx != -1 && commitIdx > migIdx) {
            migration = raw.substring(migIdx + "MIGRATION:".length(), commitIdx).trim();
            commit = raw.substring(commitIdx + "COMMIT:".length()).trim();
        }
        return new String[]{migration, commit};
    }
